...
def extract_address_custom(text):
    ...
    if address_line_match and city_state_zip_match:
        ...
        unwanted_text = "1 of 81. PARTIES. This Lease Contract"
        address = address.replace(unwanted_text, "")
...
def text_from_pdf():
    if request.method == 'POST':
        ...
        if "addresses" in selected_options:
            ...
            addresses = {}
            if address:
                addresses['custom_address'] = address
            if filtered_addresses:
                unique_addresses = list(set(filtered_addresses))
                addresses.update({f"address_{idx}": addr for idx, addr in enumerate(unique_addresses, start=1)})
